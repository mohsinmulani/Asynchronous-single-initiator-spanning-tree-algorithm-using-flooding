...
add_edge1(G,Vertex,Index,L,V) when Index == V+2  ->
	G;
...
spawner(Module,Fun,G,Vertices,Initiater) ->
    [H|T]=Vertices,
    if H == Initiater -> 
        spawn(single_initiater_bfs,bfs,[G,1,H]);
    true ->
        spawn(single_initiater_bfs,bfs,[G,-1,H])
    end,
    spawner(Module,Fun,G,T,Initiater).
...
start() ->
    ...
    {Initiater,[]} = string:to_integer(lists:nth(2,L)),
    ...
    spawner(single_initiater_bfs,bfs,G,Vertices,Initiater).